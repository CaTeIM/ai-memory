...
pub async fn run_auto_improve_review(
    ...
    let briefing = reader
        .briefing_for_project(
            ...
            // and titles of other operators' `_slots/<segment>/…` pages do
            // reach the model. Their bodies do not — only `_rules/` and
            // `procedures/` pages are loaded for patching. Narrowing this to
            // the session's operator would hide pages that exist from the
            // index, and the reviewer would propose `create` for them; the
            // exposure is a path and a title, so the trade is deliberate
