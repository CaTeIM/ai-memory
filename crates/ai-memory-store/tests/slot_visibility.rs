...
/// THE regression that shipped twice: an operator named by OIDC subject alone
/// is still a named operator. Their segment (`s-<sub>`) must admit their own
/// slot — and be distinct from an equal USERNAME's segment, so the two name
/// spaces cannot read each other's briefs.
#[tokio::test]
async fn sub_only_operator_gets_a_namespace_and_sees_their_own_slot() {
    let tmp = tempfile::tempdir().unwrap();
    let store = Store::open(tmp.path()).unwrap();
    let (ws, proj) = scope(&store).await;

    let carol = IdentityKey::Subject("oidc-subject-carol".into());
    ...
    assert!(
        ...
        "a sub-only operator must see their own slot body: {carols:?}"
    );
...
/// because the segment lands inside a SQL GLOB; `path_segment()` hex-encodes
...
async fn hostile_viewer_names_cannot_match_other_namespaces() {
    ...
    assert!(
        sneaky_ns.starts_with("ux-"),
        "a GLOB metacharacter must hex-encode: {sneaky_ns}"
    );
    ...
    assert!(
        ...
        "the hex namespace is a real, readable namespace: {seen:?}"
    );
    assert!(
...
async fn briefing_slots_follow_the_same_visibility_rule() {
    ...
    let default = paths(
        store
            .reader
            .briefing_for_project(ws, proj, 10, &SlotVisibility::default())
            .await
            .unwrap(),
    );
    assert_eq!(
    ...
    let seen = paths(
        store
            .reader
            .briefing_for_project(ws, proj, 10, &mine)
            .await
            .unwrap(),
    );
    assert!(seen.contains(&"_slots/current-focus.md".to_string()));
    ...
    let workspace_wide = paths(
        store
            .reader
            .briefing_for_workspace(ws, 10, &mine)
            .await
            .unwrap(),
    );
...
async fn recent_pages_hide_other_operators_personal_slots() {
    ...
    let project = store
        .reader
        .briefing_for_project(ws, proj, 100, &mine)
        .await
        .unwrap()
        .recent_pages;
    let workspace = store
        .reader
        .briefing_for_workspace(ws, 100, &mine)
        .await
        .unwrap()
        .recent_pages;
    let global = store
