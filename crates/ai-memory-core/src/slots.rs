...
//! values hex-encode instead of failing, so every identified operator owns a
//! namespace) and it is `[A-Za-z0-9._-]` by construction (nothing to escape,
//! nowhere to traverse).
//!
...
mod tests {
    ...
    /// names. The hex segment is what makes the refusal unnecessary: the
    /// shared slot is never touched, and the page lands somewhere its own
    /// writer can read back.
    #[test]
    fn path_hostile_identities_get_a_hex_namespace_not_the_shared_slot() {
        ...
        assert!(
            personal.starts_with("_slots/sx-"),
            "hex namespace expected: {personal}"
        );
