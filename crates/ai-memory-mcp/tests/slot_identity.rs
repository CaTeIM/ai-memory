...
fn sub_segment(sub: &str) -> String {
    IdentityKey::Subject(sub.into()).path_segment()
}
...
/// `path_segment()` is total, so the operator still owns a namespace (the hex
/// form) instead of being refused. What must NEVER happen is the fallback the
/// refusal used to guard against: landing on the shared slot every other
/// operator reads at session start. The write and the brief agree on the hex
/// namespace, so the page is readable by exactly its owner.
#[tokio::test]
async fn a_url_shaped_subject_owns_a_hex_namespace_not_the_shared_slot() {
    let h = harness(true).await;
    let url_sub = proxied("x-memory-actor-sub", "https://issuer.example/users/7");
    let ns = sub_segment("https://issuer.example/users/7");
    assert!(ns.starts_with("sx-"), "hex fallback expected: {ns}");

    let landed = write_slot(&h.http, "_slots/current-focus.md", "url-sub only", &url_sub)
        .await
        .expect("a hostile subject is re-homed into its hex namespace, not refused");
    assert_eq!(landed, format!("_slots/{ns}/current-focus.md"));

    let seen = brief_slots(&h.http, &url_sub).await;
    assert!(
        seen.contains(&landed),
        "the hex namespace must be readable by its own owner: {seen:?}",
    );
