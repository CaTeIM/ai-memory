...
# alice -> _slots/u-alice/, bob -> _slots/u-bob/, and carol — named by OIDC
# subject alone — -> _slots/s-oidc-subject-carol/.
...
echo "$C_SLOT" | grep -q "_slots/s-oidc-subject-carol/" && R=yes || R=no
check "carol (sub only) gets her s-<sub> namespace, not the shared slot" "$R" "$C_SLOT"
